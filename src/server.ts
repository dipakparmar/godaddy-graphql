...
const createGoDaddyGraphQLServer = (params?: CreateServerProps) => {
  const cors = params?.cors
  const isAllowed = params?.isAllowed
  const graphqlEndpoint = params?.graphqlEndpoint || '/v1/graphql'
  ...
  const context = (context: YogaInitialContext): Context => {
    ...
    const isAllowedFunction =
      isAllowed ||
      ((_godaddyCustomerId: string, context: Context) => {
    ...
    return createServer(createYoga({
        landingPage: false,
        cors,
        graphiql,
        graphqlEndpoint,
...
export { createGoDaddyGraphQLServer, schema }
